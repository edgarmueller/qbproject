...
class CSVAdapter(override val pathBuilders: Map[String, CSVRow => JsValue]) extends CSVSchemaAdapter {
  ...
  def parse(mainResourceIdentifier: String, schema: QBClass)(resourceMapping: (String, ResourceReference)*)
           (resourceSet: QBResourceSet): JsResult[List[JsValue]] = {
  ...
  def parseResources(mainResourceIdentifier: String, schema: QBClass)(resourceMapping: ResourceMapping)
                    (provider: QBResourceSet): JsResult[List[JsValue]] = {
  ...
  private def findMatchingData(targetId: JsValue, foreignData: JoinData): List[JsObject] = {
    ...
    foreignData.data.filter {
      ...
      case _ => println("WARN: can not resolve target " + foreignData.keys.foreignKey +
        " on primitive value")
