...
class CSVImporter(separatorChar: Char = ';', quoteChar: Char = '"') extends CSVSchemaAdapter {
   ...
   */
   def parse(schema: QBType, resource: QBResource): Either[String, List[JsValue]] = {
    val jsResult = sequenceJsResults(internalParse(schema, resource)(Set.empty))
    jsResult match {
      case JsSuccess(values, _) => Right(values)
      case err: JsError => Left(QBCSVErrorMap(err).prettyPrint)
    }
  }
  ...
  private def parseResources(primaryResourceId: String, schema: QBClass)
    ...
    validationResult match {
      ...
      case Failure(errors) => Left(errors.toList)
    }
  }
  ...
  private def parseAll(resources: List[(QBType, QBResource)], secondarySplitKeys: List[ReverseSplitKey]): Validation[NonEmptyList[QBCSVError], List[List[JsValue]]]  = {
    ...
    val results: List[Validation[NonEmptyList[QBCSVError], List[JsValue]]] = for {
      ...
      sequenceJsResults(contents) match {
        case err: JsError => QBCSVErrorMap(err).failNel
